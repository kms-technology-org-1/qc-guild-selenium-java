...
Feature: As an ESS user, I can change the password and login to workspace with new password
  ...
  Scenario Outline: <TC>. Verify that the user can change the password successfully when providing <testCases>
    ...
    Examples:
      ...
      | 01 | a new valid password which has a length greater than 8 characters | 9              | full Option       |
      | 02 | a new valid password which has a length of 8 characters           | 8              | full Option       |
      | 03 | a new valid password which has a length of 63 characters          | 63             | full Option       |
      | 04 | a new valid password which has a length of 64 characters          | 64             | full Option       |
  ...
  Scenario Outline: <TC>. Verify that the user can not change the password when providing <testCases>
    ...
    Examples:
      ...
      | 05 | a new password with a length less than 8 characters     | 7              | full Option       | Should have at least 8 characters |
      | 06 | a new password with a length greater than 64 characters | 65             | full Option       | Should not exceed 64 characters   |
  ...
  Scenario Outline: Verify that the user can not change the password when providing wrong the current password
    ...
    Examples:
      | TC | testCases                               | passwordLength | passwordCondition | message           |
      | 12 | user provide wrong the current password | 9              | full Option       | Invalid Parameter |
